...
class EgyDead :
    ...
    /**
     * The user may paste either the domain (https://example.com) or the exact
     * current landing URL (for example https://example.com/h3/).
     *
     * Content/episode links on EgyDead are usually rooted at the domain, so
     * baseUrl intentionally resolves to the origin while homeUrl preserves the
     * path the user pasted.
     */
    ...
    override fun episodeFromElement(element: Element): SEpisode {
        ...
        episode.episode_number = element.select("a").text().filter { it.isDigit() }.toFloatOrNull() ?: 0f
        return episode
    }
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        // EgyDead hides the server list until the watch form is submitted.
        val requestBody = FormBody.Builder().add("View", "1").build()
        val episodeUrl = episode.url.toAbsoluteUrl()

        val document = client.newCall(POST(episodeUrl, headers, requestBody))
            .await()
            .useAsJsoup()

        return document.select(videoListSelector()).parallelCatchingFlatMap {
            val url = it.attr("data-link").ifBlank {
                it.selectFirst("a[href]")?.attr("href").orEmpty()
            }
            if (url.isBlank()) emptyList() else extractVideos(url)
        }.distinctBy { it.videoUrl }
    }

    private suspend fun extractVideos(url: String): List<Video> = when {
        url.contains(".m3u8", true) || url.contains(".mp4", true) -> {
            listOf(Video(url, "Direct", url))
        }
        ...
        url.contains("ahvsh") -> extractSourcesScript(url, "StreamHide")

        STREAMWISH_REGEX.containsMatchIn(url) || url.contains("streamwish", true) -> {
            streamWishExtractor.videosFromUrl(url)
        }

        url.contains("fanakishtuna") -> extractSourcesScript(url, "Mirror")
        ...
        url.contains("uqload", true) -> {
            ...
            val data = request.selectFirst("script:containsData(sources)")?.data().orEmpty()
            val streamLink = data.substringAfter("sources: [\"").substringBefore("\"]")
            if (streamLink.isBlank()) emptyList() else listOf(Video(streamLink, "Uqload: Mirror", streamLink))
        }

        else -> extractGeneric(url)
    } ?: emptyList()

    private suspend fun extractSourcesScript(url: String, label: String): List<Video> {
        val request = client.newCall(GET(url, headers)).awaitSuccess().useAsJsoup()
        val script = request.selectFirst("script:containsData(sources)")?.data().orEmpty()
        val streamLink = SOURCES_REGEX.find(script)?.groupValues?.get(1).orEmpty()
        return if (streamLink.isBlank()) emptyList() else listOf(Video(streamLink, "$label: High Quality", streamLink))
    }

    /** Last-resort extractor for mirrors that changed domains but still expose a direct m3u8/mp4 in HTML. */
    private suspend fun extractGeneric(url: String): List<Video> {
        // Many StreamWish mirrors rotate domains. Try its maintained extractor first.
        try {
            val streamWish = streamWishExtractor.videosFromUrl(url)
            if (streamWish.isNotEmpty()) return streamWish
        } catch (_: Throwable) {
        }

        return try {
            val response = client.newCall(GET(url, headers)).awaitSuccess()
            val html = response.body.string().replace("\\/", "/")
            VIDEO_URL_REGEX.findAll(html)
                .map { it.value }
                .distinct()
                .map { Video(it, "Mirror", it) }
                .toList()
        } catch (_: Throwable) {
            emptyList()
        }
    ...
    override fun videoListSelector() = "ul.serversList li, .serversList li, li[data-link]"
    ...
    override fun animeDetailsParse(document: Document): SAnime {
        ...
        anime.genre = document.select("div.LeftBox li:contains(النوع) a, div.LeftBox li:contains(اللغه) a, div.LeftBox li:contains(السنه) a")
    ...
    override fun latestUpdatesRequest(page: Int): Request =
        GET(if (page <= 1) homeUrl else "$baseUrl/?page=$page", headers)
    ...
    companion object {
        ...
        private val STREAMWISH_REGEX = Regex("ajmidyad|alhayabambi|atabknh[ks]|https://.*\\.(?:sbs|top)/e/")
        private val SOURCES_REGEX = Regex("sources:\\s*\\[\\{\\s*\\t*file:\\s*[\"']([^\"']+)")
